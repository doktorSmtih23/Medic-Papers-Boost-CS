  ...
  <header className="bg-white shadow-sm">
    <div className="max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
            <span className="text-xs text-gray-500 font-mono hidden sm:block" title="Estado de la Aplicación Web Progresiva">{pwaStatus}</span>
            <button
...
export default function App() {
  ...
  const [pwaStatus, setPwaStatus] = useState<string>('PWA: Verificando...');
    ...
    if ('serviceWorker' in navigator) {
      window.addEventListener('load', () => {
        navigator.serviceWorker.register('/sw.js').then(registration => {
          ...
          console.log('Fallo en el registro del SW (esperado en este entorno): ', registrationError);
          setPwaStatus('✅ PWA configurado (Instalar desde sitio en vivo)');
        });
      });
    } else {
        setPwaStatus('❌ PWA no soportado');
    }
  }, []);
